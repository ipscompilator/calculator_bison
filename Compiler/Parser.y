...
expr: mul_expr				{ std::swap($$, $1); }
	| expr PLUS mul_expr	{ Emplace<BinaryExprNode>($$, Extract($1), Extract($3), Operation::ADD); }
	| expr MINUS mul_expr	{ Emplace<BinaryExprNode>($$, Extract($1), Extract($3), Operation::SUB); }
	;

mul_expr: unary_expr				{ std::swap($$, $1); }
	| mul_expr MULTIPLY unary_expr	{ Emplace<BinaryExprNode>($$, Extract($1), Extract($3), Operation::MUL); }
	| mul_expr DIVIDE unary_expr	{ Emplace<BinaryExprNode>($$, Extract($1), Extract($3), Operation::DIV); }
	;

unary_expr: symbol		{ std::swap($$, $1); }
	| PLUS symbol		{ Emplace<UnaryExprNode>($$, Extract($2), Operation::ADD); }
	| MINUS symbol		{ Emplace<UnaryExprNode>($$, Extract($2), Operation::SUB); }
	;
  
symbol: DOUBLE					{ $$ = new LiteralNode($1); }
